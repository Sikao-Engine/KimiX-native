...
bool rn_shlex_split(kimix::string_view text, bool posix,
    ...
    auto emit = [&]() {
        // get_token(): `if self.posix and not quoted and result == '': result = None`
...
bool rn_has_extension(kimix::string_view name,
    ...
    for (const kimix::string &ext : exts) {
        if (name.size() > ext.size()) {
            const kimix::string_view tail =
                name.substr(name.size() - ext.size());
            bool same = true;
...
kimix::string rn_repr_char(uint32_t cp) {
    ...
    default:
        if (cp < 0x20 || cp == 0x7F) {
            ...
            const bool control = (cp >= 0x0080 && cp <= 0x009F) ||
                                 cp == 0x00AD ||
                                 (cp >= 0x2000 && cp <= 0x200F) ||
                                 (cp >= 0x2028 && cp <= 0x202E) ||
                                 (cp >= 0x2060 && cp <= 0x2064) ||
...
executable_check check_executable(kimix::string_view executable,
    ...
    if (bare_python && !python_fallback.empty()) {
        const kimix::string on_path =
            which(executable, path_env, pathext, is_file);
...
env_parse_result parse_env(kimix::string_view text, bool is_string,
    ...
    for (const kimix::string &item : items) {
        ...
        if (eq == kimix::string::npos) {
            ...
            r.values.push_back(
                {kimix::string(item.substr(0, eq)), kimix::string(item.substr(eq + 1))});
        }
    }
    return r;
}
...
kimix::optional<kimix::string> validate_workdir(kimix::string_view workdir) {
    ...
    while (it < end) {
        if (static_cast<unsigned char>(*it) < 0x80u) {
            ...
            if (!rn_workdir_allowed(c)) {
                return kimix::string("Invalid workdir: character ") +
                       rn_repr_char(static_cast<uint32_t>(
        ...
        return kimix::string("Invalid workdir: character ") +
               rn_repr_char(cp) + " is not allowed.";
    }
    return std::nullopt;
}
...
kimix::string find_forbidden(kimix::string_view command,
    ...
    for (const kimix::string &keyword : keywords) {
        if (!keyword.empty() &&
            normalized.find(keyword) != kimix::string::npos) {
            return keyword;
        }
    }
    return {};
...
tool_error parse_params(const ToolParams *params, run_params &out) {
    ...
    if (params != nullptr) {
        ...
        int64_t max_lines = 0;
        err = rn_int_param(params, "max_lines", 0, k_min_max_lines,
                           std::nullopt, max_lines, present);
        if (err.failed()) {
            return err;
        }
        if (present) {
...
display_command build_display_command(kimix::string_view executable,
    ...
    d.command = (cmd_str.size() > k_huge_cmd_threshold) ? shown_executable
                                                        : cmd_str;
    return d;
}
...
tool_error Run::prepare(const run_params &params, kimix::string &output_block) {
    ...
    if (params.cwd.has_value()) {
        const kimix::optional<kimix::string> wd =
            validate_workdir(*params.cwd);
        if (wd.has_value()) {
    ...
    const resolved_command resolved =
        resolve_executable(kimix::span<const kimix::string>(parts), posix,
                           [](kimix::string_view p) {
                               return rn_default_is_file(p);
                           });
...
void Run::operator()(const ToolParams *parameters) {
    ...
    if (params.mode == "send") {
        ...
        if (!info.exists) {
            ...
            if (!all.empty()) {
                ...
                message = kimix::format(
                    "Task '{}' not found. Available tasks: [{}]", task_id,
                    kimix::string_view(names));
            }
        ...
        if (serr.failed()) {
            rn_error_result(result, serr.status,
        ...
        const proc::task_wait_result tw = proc::wait_task(
            task_id, pattern, params.timeout_seconds * 1000);
        kimix::string output;
        ...
        if (!alive && after.exit_code.has_value()) {
            ...
            block.exit_code_meaning = bash::interpret_exit_code(
                params.command, after.exit_code);
        ...
        result.values["output"] = ValueElement::make_string(
            python::build_session_output_block(block));
    ...
    if (prep.failed()) {
        const kimix::string brief =
            (prep.status == tool_status::blocked)
                ? kimix::string(prep.message.compare(0, 18, "Blocked (hardline)") == 0
                                    ? "Blocked (hardline)"
                                    : "Forbidden command")
    ...
    if (params.shell) {
#ifdef KIMIX_PLATFORM_WINDOWS
        kimix::string pwsh = _cfg.pwsh_path.empty() ? detect_pwsh_path()
                                                    : _cfg.pwsh_path;
        if (pwsh.empty()) {
...
#else
        kimix::string bash = _cfg.bash_path.empty() ? detect_bash_path()
                                                    : _cfg.bash_path;
        if (bash.empty()) {
            rn_error_result(result, tool_status::external_library,
        ...
        const kimix::string python_exe =
            _cfg.python_exe.empty() ? python::Python::detect_python_exe()
                                    : _cfg.python_exe;
        const executable_check check = check_executable(
            resolved.executable,
            [](kimix::string_view p) { return rn_default_is_file(p); },
    ...
    if (params.has_env) {
        const env_parse_result env = parse_env(
            params.env_string, params.env_is_string,
    ...
    if (params.cwd.has_value() && !params.cwd->empty()) {
        ...
        if (wd.is_relative() && _session != nullptr &&
            ...
            workdir = kimix::to_string(
                kimix::filesystem::path(_session->work_dir) / wd);
        }
    ...
    if (rr.still_running) {
        // Timeout with the child kept alive: run.py reports it as a
        // background hand-off (ToolError with the guidance message).
        ...
        const kimix::optional<kimix::string> guidance =
            kimix::runtime::tools::foreground_background_guidance(
                params.command);
        if (guidance.has_value()) {
            message = kimix::format("Running in background. task_id: `{}`. {}",
                                    kimix::string_view("(detached)"),
                                    kimix::string_view(*guidance));
        }
    ...
    if (params.output_path.has_value()) {
        ...
        if (f == nullptr) {
            rn_error_result(result, tool_status::external_library,
                            kimix::format("cannot write output_path: {}",
                                          *params.output_path),
                            "Export failed");
            rn_serialize(_result, result);
            return;
...
// Static registration: the registry key is the class name "Run" (the agent
// facing tool name in kimix.tools.file.run).
KIMIX_REGISTER_TOOL(
    Run,
