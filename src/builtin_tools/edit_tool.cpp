...
// >>> BEGIN GENERATED:ED-SPACE-TABLES >>>
...
constexpr uint32_t ed_py_space_ascii_bits[4] = {
...
// non-overlapping; do not edit by hand, run `python scripts/gen_line_hash_tables.py`.
constexpr uint32_t ed_py_space_ranges[][2] = {
    0x0085, 0x0085, 0x00A0, 0x00A0, 0x1680, 0x1680, 0x2000, 0x200A,
    0x2028, 0x2029, 0x202F, 0x202F, 0x205F, 0x205F, 0x3000, 0x3000,
};
// <<< END GENERATED:ED-SPACE-TABLES <<<
...
bool is_py_space_cp(uint32_t cp) noexcept {
    if (cp < 0x80) {
        return ((ed_py_space_ascii_bits[cp >> 5] >> (cp & 31)) & 1u) != 0;
    }
    // binary search over the sorted non-ASCII ranges
    size_t lo = 0;
    size_t hi = sizeof(ed_py_space_ranges) / sizeof(ed_py_space_ranges[0]);
    while (lo < hi) {
        const size_t mid = lo + (hi - lo) / 2;
        if (cp < ed_py_space_ranges[mid][0]) {
            hi = mid;
        } else if (cp > ed_py_space_ranges[mid][1]) {
            lo = mid + 1;
        } else {
            return true;
        }
...
// Generated by scripts/gen_line_hash_tables.py - the same table as
// runtime/tools/line_hash.cpp and read_tool.cpp's rd_alnum_ranges.
// ---------------------------------------------------------------------------
// >>> BEGIN GENERATED:ED-ALNUM-TABLE >>>
...
constexpr uint32_t ed_alnum_ranges[][2] = {
...
bool is_alnum_cp(uint32_t cp) noexcept {
    ...
    size_t lo = 0;
    size_t hi = sizeof(ed_alnum_ranges) / sizeof(ed_alnum_ranges[0]);
    while (lo < hi) {
        const size_t mid = lo + (hi - lo) / 2;
        if (cp < ed_alnum_ranges[mid][0]) {
            hi = mid;
        } else if (cp > ed_alnum_ranges[mid][1]) {
            lo = mid + 1;
        } else {
            return true;
        }
...
fuzzy_match_result best_fuzzy_match(kimix::string_view target,
    ...
    if (best_score >= cutoff) {
        ...
        // _find_best_fuzzy_match (replace.py:140-143) returns None when the
        // best score is below the cutoff, so a below-cutoff candidate must not
        // leak out through matched_original (the caller treats a present
        // matched_original as "fuzzy match found").
        r.matched_original.reset();
    }
    return r;
}
...
hunks_result parse_diff_hunks(kimix::string_view diff) {
    ...
    for (const kimix::string &raw_line : raw_lines) {
        ...
        if (!current.has_value()) {
            ...
            // diff.py: {line[:80]!r} -- a CODE POINT slice, not a byte slice
            // (a byte slice cut multi-byte characters in half and shortened
            // the message).
            const kimix::string shown =
                py_repr(edit_detail::first_80_cps(kimix::string_view(line)));
            r.error.message = kimix::format(
                "Unexpected diff content outside a hunk: {}",
                kimix::string_view(shown));
            return r;
        }
