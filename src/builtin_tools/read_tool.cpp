...
uint32_t rd_decode(const char *&it, const char *end) noexcept {
    ...
    // Unicode "maximal subpart" rule (CPython's errors="replace" decoder): the
    // ill-formed subsequence is the longest *valid prefix*, and it decodes to a
    // single U+FFFD. So the two bytes E6 B1 yield one replacement character
    // (not two) while E6 followed by 'A' yields one replacement for the lead
    // byte, then 'A'.
    const size_t need = extra + 1;
    size_t have = 1;
    while (have < need && have < avail &&
           (static_cast<uint8_t>(it[have]) & 0xC0u) == 0x80u) {
        have++;
    }
    if (have < need) {
        it += have;
        return 0xFFFD; // truncated / ill-formed continuation
    }
    const char *saved = it;
    for (size_t i = 1; i < need; i++) {
        cp = (cp << 6) | (static_cast<uint8_t>(it[i]) & 0x3Fu);
    }
    it += need;
    // Overlong / surrogate / out-of-range checks (consume just the lead byte,
    // exactly like CPython's decoder does for these).
...
// Start byte of the code point that ends at `pos` (walks back over at most
// three continuation bytes; used by the markdown heading pass to backtrack
// `\s*` one character at a time, exactly like the regex engine).
size_t rd_prev_code_point_start(kimix::string_view text, size_t pos) noexcept {
    if (pos == 0) {
        return 0;
    }
    size_t start = pos - 1;
    ...
    while (start > 0 && steps < 3 &&
           (static_cast<uint8_t>(text[start]) & 0xC0u) == 0x80u) {
        start--;
        steps++;
    }
    return start;
...
// Python's inline-code placeholder, read_markit.py:229 f"\x00CODE{n}\x00".
// The leading NUL has to be built as a real byte: a plain "\x00CODE" literal
// parses as the greedy hex escape "\x0CODE" (0x0C + "ODE"), and 0x0C is
// Python whitespace, which then changes the heading pass and the final strip.
kimix::string rd_inline_code_token(size_t index) {
    kimix::string token;
    token.push_back('\0');
    token += "CODE";
    token += std::to_string(index).c_str();
    token.push_back('\0');
    return token;
}
...
// Python str.isspace() code-point set (exact: this is also the set `re` treats
// as \s for str patterns, i.e. SRE_UNI_IS_SPACE == Py_UNICODE_ISSPACE, and the
// set str.strip() removes). Note U+001C..U+001F: they are space per Python but
// are *not* ASCII whitespace.
...
// >>> BEGIN GENERATED:RD-ALNUM-TABLE >>>
...
// Word character for the markdown underscore lookarounds.
//
// read_markit.py imports the *third-party* `regex` module (`import regex as
// re`), whose \w is \p{Word} (the UTS#18 word property). Measured against the
// vendored regex version that means: L*, N* except No, Mn, Mc, Me, Pc, the two
// join controls U+200C/U+200D and (surprisingly) unassigned code points are
// word characters, while No characters such as U+00B2 are NOT. This helper
// keeps the port's documented approximation: alnum (str.isalnum, i.e. L*+N*)
// plus '_' plus connector punctuation. That is exact for ASCII, CJK and
// European text (the plan's ASCII fast-path convention) and covers the common
// intraword-underscore cases; combining marks (Mn/Mc/Me) and the join controls
// still differ — see reports/read.md deviation 6.
...
bool render_cpu_profile(kimix::string_view json_text, kimix::string &out) {
    ...
    // ── Root promotion ──────────────────────────────────────────────────────
    // read_profiles.py::_promote_root: only a *missing* or JSON-null "root"
    // falls back to the "(root)" functionName heuristic; any other value is
    // looked up in the node map (a miss leaves root = None, which sends the hot
    // paths section down the "top self-time functions" branch).
    size_t root_idx = SIZE_MAX;
    yyjson_val *root_val = yyjson_obj_get(profile, "root");
    const bool root_present =
        root_val != nullptr && yyjson_get_type(root_val) != YYJSON_TYPE_NULL;
    if (!root_present) {
        for (size_t i = 0; i < arena.size(); i++) {
        ...
        if (root_idx == SIZE_MAX && !arena.empty()) {
            // _promote_root: next(iter(node_map.values())) - the first node.
            root_idx = 0;
        }
    } else if (yyjson_is_num(root_val)) {
        const double root_num = yyjson_get_num(root_val);
        if (root_num == static_cast<double>(static_cast<int64_t>(root_num))) {
            const auto found =
                id_to_idx.find(static_cast<int64_t>(root_num));
            if (found != id_to_idx.end()) {
...
kimix::string markdown_to_text(kimix::string_view md) {
    ...
    kimix::vector<kimix::string> inline_code;
    {
        ...
        while (pos < text.size()) {
            ...
            if (close >= text.size() || close == open + 1) {
                // `([^`]+)` requires at least one non-backtick char: emit the
                // text up to and including this backtick and retry from the
                // next character (the regex engine advances by one).
            ...
            out.append(rd_inline_code_token(inline_code.size() - 1));
            pos = close + 1;
        }
        text = std::move(out);
    }
    ...
    // 3. Bold: \*\*([^*]+)\*\* -> \1
    {
        ...
        while (true) {
            ...
            if (i + 1 < text.size() && text[i + 1] == '*' && i > content_start) {
                ...
                // No match at `open`: the regex engine advances the start by
                // ONE character, so a match may still begin at open + 1
                // ("***a**" -> "*a").
                out.append(text.data() + pos, open + 1 - pos);
                pos = open + 1;
            }
    ...
    // 5. Double-underscore bold: __([^_]+)__ -> \1
    {
        ...
        while (true) {
            ...
            if (i + 1 < text.size() && text[i + 1] == '_' && i > content_start) {
                ...
                // Same single-character retry as the "**" pass ("___a__" ->
                // "_a").
                out.append(text.data() + pos, open + 1 - pos);
                pos = open + 1;
            }
    ...
    //    pass below only sees what remains, matching Python order.)
    {
        ...
        while (pos < text.size()) {
            ...
            if (paren_end >= text.size() || paren_end == close + 2) {
                // `\(([^)]+)\)` needs at least one URL character: "[a]()" is
                // left untouched by the reference.
                out.append(text.data() + pos, open + 1 - pos);
                pos = open + 1;
                continue;
            }
    ...
    // 8. Images: !\[([^\]]*)\]\(([^)]+)\) -> "[image: \2]"
    {
        ...
        while (pos < text.size()) {
            ...
            if (paren_end >= text.size() || paren_end == close + 2) {
                // Empty URL: "[^)]+" requires one character, so "![]()" stays.
                out.append(text.data() + pos, open + 2 - pos);
                pos = open + 2;
                continue;
            }
    ...
    //
    // Faithful port of the regex, including its backtracking and the fact that
    // Python's `\s` matches '\n' (so `\s*` may run into the following lines):
    //   "# \nx"       -> "x"          (\s* eats the space + the newline, "x"
    //                                  becomes group 1)
    //   "a\n####\nb"  -> "a\nb"       (same cross-line consumption)
    //   "####"        -> "#"          (#+ backtracks so (.+) gets the last '#')
    //   "##\n"        -> "#"
    //   "#\n"         -> unchanged    (no match: (.+) cannot match '\n')
    {
        kimix::string out;
        out.reserve(text.size());
        const size_t n = text.size();
        size_t pos = 0;   // first byte not yet copied into `out`
        size_t scan = 0;  // candidate `^` position
        while (scan < n) {
            ...
            size_t hash_end = scan;
            while (hash_end < n && text[hash_end] == '#') {
                hash_end++;
            }
            bool found = false;
            size_t match_end = 0;
            size_t keep_from = 0;
            for (size_t k = hash_end; !found && k > scan; k--) {
                // greedy \s* from k
                size_t ws_end = k;
                while (ws_end < n) {
                    const char *it = text.data() + ws_end;
                    const char *before = it;
                    const uint32_t cp = rd_decode(it, text.data() + n);
                    if (!rd_is_space_cp(cp)) {
                        break;
                    }
                    ws_end += static_cast<size_t>(it - before);
                }
                // \s* backtracks one code point at a time, then (.+) must reach
                ...
                while (true) {
                    ...
                    j = rd_prev_code_point_start(text, j);
                }
            }
            if (!found) {
                scan++;
            ...
            out.append(text.data() + pos, scan - pos);
            out.append(text.data() + keep_from, match_end - keep_from);
            pos = match_end;
            scan = match_end;
        }
        out.append(text.data() + pos, n - pos);
        text = std::move(out);
    }
    ...
    for (size_t i = 0; i < inline_code.size(); i++) {
        const kimix::string token = rd_inline_code_token(i);
        size_t pos = 0;
