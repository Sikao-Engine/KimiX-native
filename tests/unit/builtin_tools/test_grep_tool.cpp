...
// ---------------------------------------------------------------------------
// Golden-vector harness (tests/unit/builtin_tools/grep_goldens.inc)
// ---------------------------------------------------------------------------
// grep_goldens.inc is GENERATED by scripts/gen_grep_goldens.py, which runs the
// kimi-agent reference implementation (kimi-cli/src/kimi_cli/tools/file/*.py,
// utils/sensitive.py) over the corpus and stores the answers. Encoding:
//   input - TAB-separated fields; each field is 4-escape encoded
//           (backslash backslash -> '\', backslash n -> LF, backslash r -> CR,
//           backslash t -> TAB), so a raw TAB byte always separates fields;
//   want  - a JSON-ish value rendered with the same 4-escape rule (quotes are
//           NOT escaped - the comparand is produced byte-identically here).
// Line-list inputs carry an explicit count field first so that an empty list
// and a single empty line stay distinguishable.
...
constexpr size_t g_n(const T (&)[N]) noexcept {
    return N;
}

std::string gunesc(std::string_view s) {
    std::string out;
    out.reserve(s.size());
...
std::vector<std::string> gfields(std::string_view in) {
    std::vector<std::string> out;
    size_t pos = 0;
    while (true) {
        const size_t tab = in.find('\t', pos);
        const size_t stop = tab == std::string_view::npos ? in.size() : tab;
        out.push_back(gunesc(in.substr(pos, stop - pos)));
...
std::string gjstr(std::string_view s) {
    std::string out;
...
std::string gjstr_list(const Range &v) {
    std::string out = "[";
    bool first = true;
    for (const auto &item : v) {
        ...
        first = false;
        out += gjstr(std::string_view(item.data(), item.size()));
    }
    out += "]";
    return out;
}

std::string gjint_list(kimix::span<const uint32_t> v) {
    std::string out = "[";
...
std::string gjbool(bool b) { return b ? "true" : "false"; }

std::string g_unsupported() { return "\"unsupported\""; }

size_t g_count(const std::vector<std::string> &f, size_t at) {
    if (at >= f.size() || f[at].empty()) {
        return 0;
    }
    return static_cast<size_t>(std::strtoul(f[at].c_str(), nullptr, 10));
}

uint32_t g_u32(const std::string &s) {
    return static_cast<uint32_t>(std::strtoul(s.c_str(), nullptr, 10));
}

std::vector<std::string> g_lines(const std::vector<std::string> &f, size_t count_at) {
    std::vector<std::string> out;
    const size_t n = g_count(f, count_at);
    for (size_t i = 0; i < n && count_at + 1 + i < f.size(); i++) {
        out.push_back(f[count_at + 1 + i]);
    }
    return out;
}

std::string g_range_text(const line_range &r) {
    std::string out = std::to_string(r.start_line) + "-";
    out += r.end_line.has_value() ? std::to_string(*r.end_line) : std::string("open");
    return out;
}

// parse_line_range_chunk golden: "none" | "err:<ValueError text>" | "S-E".
std::string g_sresult_json(const g::selector_result &r) {
    if (r.status == tool_status::unsupported) {
        return g_unsupported();
    }
    if (r.status != tool_status::ok) {
        return gjstr("err:" + std::string(r.message.data(), r.message.size()));
    }
    if (r.no_value || r.ranges.empty()) {
        return "\"none\"";
    }
    return gjstr(g_range_text(r.ranges.front()));
}

// parse_line_ranges / selector_line_ranges golden: "none" | "err:..." | [..].
std::string g_slist_json(const g::selector_result &r) {
    if (r.status == tool_status::unsupported) {
        return g_unsupported();
    }
    if (r.status != tool_status::ok) {
    ...
    for (size_t i = 0; i < r.ranges.size(); i++) {
        ...
        out += gjstr(g_range_text(r.ranges[i]));
    }
    return out + "]";
}
...
std::string g_a_lr_chunk(const std::vector<std::string> &f) {
    return g_sresult_json(g::parse_line_range_chunk(f[0]));
}

std::string g_a_lr_ranges(const std::vector<std::string> &f) {
    return g_slist_json(g::parse_line_ranges(f[0]));
}

std::string g_a_sel_ranges(const std::vector<std::string> &f) {
    return g_slist_json(g::selector_line_ranges(f[0]));
}

std::string g_a_line_in_ranges(const std::vector<std::string> &f) {
    const g::selector_result rr = g::parse_line_ranges(f[0]);
    if (rr.failed()) {
        return gjstr("bad-spec");
    }
    // Python's None (no chunk parsed) hits is_line_in_ranges(line, None) -> True,
    // which is the empty-span contract of the native kernel.
    return gjbool(g::is_line_in_ranges(g_u32(f[1]), rr.ranges));
}

std::string g_a_split_sel(const std::vector<std::string> &f) {
    const bool exists = f.size() > 1 && f[1] == "1";
    kimix::function<bool(kimix::string_view)> probe = [exists](kimix::string_view) noexcept {
        return exists;
    };
    g::path_selector ps;
    if (g::split_path_and_sel(f[0], probe, ps) != tool_status::ok) {
        return g_unsupported();
    }
    std::string out = "[" + gjstr(ps.path) + ",";
    out += ps.has_selector ? gjstr(ps.selector) : std::string("null");
    return out + "]";
}

std::string g_a_expand_str(const std::vector<std::string> &f) {
    kimix::vector<kimix::string> out;
    if (g::expand_path_entries(f[0], out) != tool_status::ok) {
        return g_unsupported();
    }
    return gjstr_list(out);
}

std::string g_a_expand_list(const std::vector<std::string> &f) {
    const size_t n = g_count(f, 0);
    kimix::vector<kimix::string> items;
    items.reserve(n);
    for (size_t i = 0; i < n && 1 + i < f.size(); i++) {
        items.emplace_back(f[1 + i].data(), f[1 + i].size());
    }
    kimix::vector<kimix::string> out;
    if (g::expand_path_entries(items, out) != tool_status::ok) {
        return g_unsupported();
    }
    return gjstr_list(out);
}

std::string g_a_parse_content(const std::vector<std::string> &f) {
    g::content_line parsed;
    bool no_match = false;
    if (g::parse_content_line(f[0], parsed, no_match) == tool_status::unsupported) {
        return g_unsupported();
    }
    if (no_match) {
        return "\"none\"";
    }
    std::string out = "{\"path\":" + gjstr(parsed.path);
    out += ",\"line_no\":" + std::to_string(parsed.line_no);
    out += ",\"is_match\":" + gjbool(parsed.is_match);
    out += ",\"text\":" + gjstr(parsed.text) + "}";
    return out;
}

std::string g_a_line_shape(const std::vector<std::string> &f) {
    size_t path_len = 0;
    bool no_match = false;
    if (g::line_path_shape(f[0], path_len, no_match) == tool_status::unsupported) {
        return g_unsupported();
    }
    return no_match ? std::string("\"none\"") : std::to_string(path_len);
}

std::string g_a_fmt_match(const std::vector<std::string> &f) {
    kimix::string out;
    g::format_match_line(g_u32(f[0]), f[1], f[2] == "1", out);
    return gjstr(out);
}

std::string g_a_group_files(const std::vector<std::string> &f) {
    const std::vector<std::string> lines = g_lines(f, 0);
    kimix::vector<g::file_group> groups;
    if (g::group_lines_by_file(kv(lines), groups) != tool_status::ok) {
        return g_unsupported();
    }
    std::string out = "[";
    for (size_t i = 0; i < groups.size(); i++) {
        ...
        out += "[" + gjstr(groups[i].path) + ",[";
        for (size_t j = 0; j < groups[i].body.size(); j++) {
            const g::grouped_entry &e = groups[i].body[j];
            if (j != 0) {
                out += ",";
            }
            out += "[" + std::to_string(e.line_no) + "," + gjbool(e.is_match) + "," +
                   gjstr(e.text) + "]";
        }
        out += "]]";
    }
...
std::string g_a_fmt_grouped(const std::vector<std::string> &f) {
    const std::vector<std::string> lines = g_lines(f, 0);
    kimix::vector<g::file_group> groups;
    if (g::group_lines_by_file(kv(lines), groups) != tool_status::ok) {
        return g_unsupported();
    ...
    kimix::vector<kimix::string> out;
    g::format_grouped_output(groups, out);
    return gjstr_list(out);
}

std::string g_a_group_blank(const std::vector<std::string> &f) {
    kimix::vector<kimix::vector<uint32_t>> groups;
    g::group_line_indices_by_blank(kv(g_lines(f, 0)), groups);
    std::string out = "[";
    for (size_t i = 0; i < groups.size(); i++) {
        if (i != 0) {
            out += ",";
        }
        out += gjint_list(groups[i]);
    }
    return out + "]";
}

std::string g_a_range_filter(const std::vector<std::string> &f) {
    const size_t npairs = g_count(f, 0);
    g::ranges_map map;
    size_t at = 1;
    for (size_t i = 0; i < npairs; i++) {
        const std::string &key = f[at];
        const g::selector_result rr = g::parse_line_ranges(f[at + 1]);
        at += 2;
        if (rr.failed()) {
            return gjstr("bad-spec");
        }
        g::merge_ranges_into(map, key, rr.ranges);
    }
    const std::vector<std::string> lines = g_lines(f, at);
    kimix::vector<kimix::string> out;
    if (g::range_filter_lines(kv(lines), map, out) != tool_status::ok) {
        return g_unsupported();
    }
    return gjstr_list(out);
}

std::string g_a_remap(const std::vector<std::string> &f) {
    const size_t npairs = g_count(f, 0);
    g::display_map map;
    size_t at = 1;
    for (size_t i = 0; i < npairs; i++) {
        g::display_entry entry;
        entry.scratch.assign(f[at].data(), f[at].size());
        entry.display.assign(f[at + 1].data(), f[at + 1].size());
        map.push_back(std::move(entry));
        at += 2;
    }
    const std::vector<std::string> lines = g_lines(f, at);
    kimix::vector<kimix::string> out;
    g::remap_display(kv(lines), map, out);
    return gjstr_list(out);
}

std::string g_a_reattach(const std::vector<std::string> &f) {
    const std::vector<std::string> lines = g_lines(f, 1);
    kimix::vector<kimix::string> out;
    if (g::reattach_single_file_prefix(kv(lines), f[0], out) != tool_status::ok) {
        return g_unsupported();
    }
    return gjstr_list(out);
}

std::string g_a_strip_prefix(const std::vector<std::string> &f) {
    const std::vector<std::string> lines = g_lines(f, 1);
    kimix::vector<kimix::string> out;
    g::strip_path_prefix(kv(lines), f[0], out);
    return gjstr_list(out);
}

std::string g_a_normalize(const std::vector<std::string> &f) {
    const std::vector<std::string> lines = g_lines(f, 2);
    kimix::vector<kimix::string> out;
    if (g::normalize_slashes_content(kv(lines), f[0], f[1] == "1", out) != tool_status::ok) {
        return g_unsupported();
    }
    return gjstr_list(out);
}

std::string g_a_collect(const std::vector<std::string> &f) {
    const std::vector<std::string> lines = g_lines(f, 1);
    kimix::vector<kimix::string> out;
    if (g::collect_record_files(kv(lines), f[0], out) != tool_status::ok) {
        return g_unsupported();
    }
    return gjstr_list(out);
}

std::string g_a_rtk_parse(const std::vector<std::string> &f) {
    const std::vector<std::string> lines = g_lines(f, 0);
    kimix::vector<kimix::string> cleaned;
    g::rtk_meta meta;
    if (g::parse_rtk_rg_output(kv(lines), cleaned, meta) != tool_status::ok) {
        return g_unsupported();
    }
    std::string out = "{\"cleaned\":" + gjstr_list(cleaned);
    out += ",\"total_matches\":";
    out += meta.total_matches.has_value() ? std::to_string(*meta.total_matches)
                                          : std::string("null");
    out += ",\"total_files\":";
    out += meta.total_files.has_value() ? std::to_string(*meta.total_files) : std::string("null");
    out += ",\"folded_files\":[";
    for (size_t i = 0; i < meta.folded_files.size(); i++) {
        const g::rtk_folded_file &ff = meta.folded_files[i];
        if (i != 0) {
            out += ",";
        }
        out += "{\"path\":" + gjstr(ff.path);
        out += ",\"count\":" + std::to_string(ff.count);
        out += ",\"log\":" + (ff.has_log ? gjstr(ff.log) : std::string("null"));
        out += ",\"start_line\":";
        out += ff.start_line.has_value() ? std::to_string(*ff.start_line) : std::string("null");
        out += "}";
    }
    out += "],\"skipped_files\":";
    out += meta.skipped_files.has_value() ? std::to_string(*meta.skipped_files)
                                          : std::string("null");
    out += ",\"skipped_log\":";
    out += meta.has_skipped_log ? gjstr(meta.skipped_log) : std::string("null");
    return out + "}";
}

std::string g_a_rtk_note(const std::vector<std::string> &f) {
    const std::vector<std::string> lines = g_lines(f, 1);
    kimix::vector<kimix::string> cleaned;
    g::rtk_meta meta;
    g::parse_rtk_rg_output(kv(lines), cleaned, meta);
    kimix::string note;
    g::rtk_fold_note(meta, f[0], note);
    return note.empty() ? std::string("null") : gjstr(note);
}

std::string g_a_rec_record(const std::vector<std::string> &f) {
    kimix::vector<kimix::string> existing;
    for (const std::string &p : f) {
        g::recorder_record(existing, p);
    }
    return gjstr_list(existing);
}

std::string g_a_rec_merge(const std::vector<std::string> &f) {
    const size_t cap = g_count(f, 0);
    const size_t n_existing = g_count(f, 1);
    size_t at = 2;
    kimix::vector<kimix::string> existing;
    for (size_t i = 0; i < n_existing && at + i < f.size(); i++) {
        existing.emplace_back(f[at + i].data(), f[at + i].size());
    }
    at += n_existing;
    const size_t n_fresh = g_count(f, at);
    at += 1;
    kimix::vector<kimix::string> fresh;
    for (size_t i = 0; i < n_fresh && at + i < f.size(); i++) {
        fresh.emplace_back(f[at + i].data(), f[at + i].size());
    }
    kimix::vector<kimix::string> out;
    g::recorder_merge(existing, fresh, cap, out);
    return gjstr_list(out);
}

std::string g_a_sensitive(const std::vector<std::string> &f) {
    bool out = false;
    if (g::is_sensitive_path(f[0], true, out) != tool_status::ok) {
        return g_unsupported();
    }
    return gjbool(out);
}

std::string g_a_warning(const std::vector<std::string> &f) {
    const size_t n = g_count(f, 0);
    kimix::vector<kimix::string> paths;
    for (size_t i = 0; i < n && 1 + i < f.size(); i++) {
        paths.emplace_back(f[1 + i].data(), f[1 + i].size());
    }
    kimix::string out;
    if (g::sensitive_file_warning(paths, true, out) != tool_status::ok) {
        return g_unsupported();
    }
    return gjstr(out);
}

std::string g_a_join_limit(const std::vector<std::string> &f) {
    const size_t max_bytes = g_count(f, 0);
    const std::vector<std::string> lines = g_lines(f, 1);
    kimix::string out;
    bool truncated = false;
    size_t omitted = 0;
    if (!g::join_with_byte_limit(kv(lines), max_bytes, out, truncated, omitted)) {
        return "\"invalid-utf8\"";
    }
    return "[" + gjstr(out) + "," + gjbool(truncated) + "]";
}

std::string g_a_pattern_newline(const std::vector<std::string> &f) {
    bool out = false;
    if (g::pattern_has_regex_newline(f[0], out) != tool_status::ok) {
        return g_unsupported();
    }
    return gjbool(out);
}
...
template <class Fn>
void g_run(const char *name, const grep_golden *rows, size_t n, Fn fn) {
    for (size_t i = 0; i < n; i++) {
        const std::vector<std::string> f = gfields(rows[i].input);
        const std::string got = fn(f);
        expect(got == rows[i].want)
            << std::string(name) << " case " << static_cast<unsigned long long>(i) << "\n  in   = "
            << rows[i].input << "\n  want = " << rows[i].want << "\n  got  = " << got;
    }
}

#include "grep_goldens.inc"
...
x])", {"[\nx]"}}, // strict JSON with a newline is valid
    ...
    // ------------------------------------------------ Python-derived goldens
    // Every vector below was generated by scripts/gen_grep_goldens.py running
    // the kimi-agent reference implementation over the corpus (see the header
    // of tests/unit/builtin_tools/grep_goldens.inc for the encoding).
    // ------------------------------------------------------ parity regressions
    // Each block is a divergence the golden corpus caught while it was being
    // built (the pre-fix native answer and the reference answer are both
    // quoted); they stay as explicit, readable regression tests.
    "parity_regressions"_test = [] {
        // 1. expand_path_entries('['-string) with unicode entries.
        //    Reference: orjson decodes them -> ["café.py", "b.py"].
        //    Before: the JSON branch was skipped for non-ASCII input, so the
        //    whole string was returned as one ';'-entry.
        {
            kimix::vector<kimix::string> out;
            expect(g::expand_path_entries("[\"caf\xc3\xa9.py\",\"b.py\"]", out) == tool_status::ok);
            expect(((sv(out)) == (std::vector<std::string>{"caf\xc3\xa9.py", "b.py"})));
        }
        // 2. Adjacent-range merge at the uint32 ceiling.
        //    Reference: [4294967295-4294967295] (4294967295 <= end + 1).
        //    Before: `end + 1u` wrapped to 0 -> two separate ranges.
        {
            const g::selector_result r =
                g::parse_line_ranges("4294967295-4294967295,4294967295-4294967295");
            expect(((ranges_text(r.ranges)) == (std::string("4294967295-4294967295"))));
        }
        // 3. join_with_byte_limit: the newline separator depends on how many
        //    lines were collected, not on the joined buffer being non-empty.
        //    Reference: (["", "b"], 1000) -> "\nb".
        //    Before: the leading empty line left the buffer empty -> "b".
        {
            kimix::vector<kimix::string> lines = kv({"", "b"});
            kimix::string out;
            bool truncated = false;
            size_t omitted = 0;
            expect(g::join_with_byte_limit(lines, 1000, out, truncated, omitted));
            expect(((s_of(out)) == (std::string("\nb"))));
            expect(!truncated);
        }
        // 4. rtk regexes are anchored with `$`, which also matches before ONE
        //    trailing newline (a caller that split the stream itself).
        //    Reference: skipped_files=133, skipped_log="log".
        {
            kimix::vector<kimix::string> cleaned;
            g::rtk_meta meta;
            expect(g::parse_rtk_rg_output(kv({"+133 more files [see remaining: log]\n"}), cleaned,
                                          meta) == tool_status::ok);
            expect(cleaned.empty());
            expect(meta.skipped_files.has_value() && *meta.skipped_files == 133u);
            expect(meta.has_skipped_log);
            expect(((s_of(meta.skipped_log)) == (std::string("log"))));
        }
        // 5. `_parse_tail_hint`'s `(\S+)$` rejects every Python-\s byte, not only
        //    space/tab. Reference: the hint does not parse, so the fold note
        //    keeps the raw hint text and no start line.
        {
            const std::string text = "+1 more in p.py [see remaining: tail -n +5 lo\x0b" "g]";
            const std::string hint = "tail -n +5 lo\x0b" "g";
            kimix::vector<kimix::string> cleaned;
            g::rtk_meta meta;
            expect(g::parse_rtk_rg_output(kv({text}), cleaned, meta) == tool_status::ok);
            expect(((meta.folded_files.size()) == (size_t(1))));
            expect(!meta.folded_files[0].start_line.has_value());
            expect(((s_of(meta.folded_files[0].log)) == (hint)));
        }
    };

    // ------------------------------------------------- native_io branch (repro)
    // Session::native_io (set by src/agent/soul.cpp) makes Grep run its own
    // simplified ripgrep (regex_lite + a filesystem walk) instead of answering
    // "unsupported". That branch is NOT a drop-in for the Python tool, and this
    // test pins the differences (see src/builtin_tools/reports/grep.md
    // "native_io branch"):
    //   * files/paths are the walk paths (absolute), while the Python tool
    //     strips the search base (_strip_path_prefix) and prints display paths;
    //   * the message is "{N} match(es) in {M} file(s)", not the Python
    //     builder's message (which merges counts and rtk fold notes);
    //   * hidden entries are skipped at every depth and .gitignore is never
    //     consulted (ripgrep skips ignored files, and `include_ignored`
    //     re-enables them);
    //   * -B/-A/-C, include, head_limit and output_mode are the only supported
    //     parameters; group/record/sensitive/offset/fold/multiline are ignored;
    //   * the matcher is regex_lite (documented subset), not the Python `regex`
    //     engine - the reason the line-search kernel is gated (issue/grep.md).
    "grep_tool_native_io_branch_contract"_test = [] {
        namespace fs = kimix::filesystem;
        std::error_code ec;
        const fs::path root = fs::temp_directory_path(ec) / "kimix_grep_native_io";
        ...
        put(root / "a.py", "hit\nmiss\n");
        put(root / "sub" / "b.py", "miss\nhit\n");
        put(root / ".hidden.py", "hit\n");
        ...
        auto run = [&](const char *mode, const kimix::builtin_tools::ToolParams *extra) {
            g::Grep tool(&session);
            kimix::builtin_tools::ToolParams params;
            ...
            if (mode != nullptr) {
                params.values["output_mode"] = ValueElement::make_string(kimix::string(mode));
            }
            if (extra != nullptr) {
        ...
        // files_with_matches: absolute walk paths, "{N} match(es) in {M} file(s)"
        {
            kimix::builtin_tools::ToolParams result = run("files_with_matches", nullptr);
            expect(((s_of(result.get("status")->as_string())) == (std::string("ok"))));
            const ValueElement *message = result.get("message");
            expect(((s_of(message->as_string())) == (std::string("2 match(es) in 2 file(s)"))));
            const ValueElement *files = result.get("files");
            expect(((files->as_array().size()) == (size_t(2))));
            for (const ValueElement &f : files->as_array()) {
                const std::string path = s_of(f.as_string());
                // the search base is NOT stripped (the Python tool returns "a.py")
                expect(path.find(root_text) == 0) << path;
                expect(path.find(".hidden.py") == std::string::npos) << path;
            }
        }
        // content mode: rg-style "path:LN:text" with absolute walk paths
        {
            kimix::builtin_tools::ToolParams result = run("content", nullptr);
            const std::string output = s_of(result.get("output")->as_string());
            expect(output.find("a.py:1:hit") != std::string::npos) << output;
            expect(output.find("b.py:2:hit") != std::string::npos) << output;
            expect(output.find(".hidden.py") == std::string::npos) << output;
        }
        // count_matches
        {
            kimix::builtin_tools::ToolParams result = run("count_matches", nullptr);
            const std::string output = s_of(result.get("output")->as_string());
            expect(output.find("a.py:1") != std::string::npos) << output;
            expect(output.find("b.py:1") != std::string::npos) << output;
        }
        // -C context lines use the '-' delimiter, matches keep ':'
        {
            kimix::builtin_tools::ToolParams extra;
            extra.values["-C"] = ValueElement::make_int(1);
            kimix::builtin_tools::ToolParams result = run("content", &extra);
            const std::string output = s_of(result.get("output")->as_string());
            expect(output.find("a.py-2-miss") != std::string::npos) << output;
            expect(output.find("a.py:1:hit") != std::string::npos) << output;
        }
        fs::remove_all(root, ec);
    };
    ...
    "goldens_selectors"_test = [] {
        ...
        g_run("sel_ranges", k_g_golden_sel_ranges, g_n(k_g_golden_sel_ranges), g_a_sel_ranges);
        g_run("line_in_ranges", k_g_golden_line_in_ranges, g_n(k_g_golden_line_in_ranges),
              g_a_line_in_ranges);
        g_run("split_sel", k_g_golden_split_sel, g_n(k_g_golden_split_sel), g_a_split_sel);
        g_run("expand_str", k_g_golden_expand_str, g_n(k_g_golden_expand_str), g_a_expand_str);
        g_run("expand_list", k_g_golden_expand_list, g_n(k_g_golden_expand_list), g_a_expand_list);
    };

    "goldens_content_lines_and_rendering"_test = [] {
        g_run("parse_content", k_g_golden_parse_content, g_n(k_g_golden_parse_content),
              g_a_parse_content);
        g_run("line_shape", k_g_golden_line_shape, g_n(k_g_golden_line_shape), g_a_line_shape);
        g_run("fmt_match", k_g_golden_fmt_match, g_n(k_g_golden_fmt_match), g_a_fmt_match);
        g_run("group_files", k_g_golden_group_files, g_n(k_g_golden_group_files), g_a_group_files);
        g_run("fmt_grouped", k_g_golden_fmt_grouped, g_n(k_g_golden_fmt_grouped), g_a_fmt_grouped);
        g_run("group_blank", k_g_golden_group_blank, g_n(k_g_golden_group_blank), g_a_group_blank);
        g_run("range_filter", k_g_golden_range_filter, g_n(k_g_golden_range_filter), g_a_range_filter);
        g_run("remap_display", k_g_golden_remap_display, g_n(k_g_golden_remap_display), g_a_remap);
        g_run("reattach", k_g_golden_reattach, g_n(k_g_golden_reattach), g_a_reattach);
        g_run("strip_prefix", k_g_golden_strip_prefix, g_n(k_g_golden_strip_prefix),
              g_a_strip_prefix);
        g_run("normalize_slashes", k_g_golden_normalize_slashes,
              g_n(k_g_golden_normalize_slashes), g_a_normalize);
        g_run("collect_files", k_g_golden_collect_files, g_n(k_g_golden_collect_files), g_a_collect);
        g_run("join_limit", k_g_golden_join_limit, g_n(k_g_golden_join_limit), g_a_join_limit);
    };

    "goldens_rtk_protocol"_test = [] {
        g_run("rtk_parse", k_g_golden_rtk_parse, g_n(k_g_golden_rtk_parse), g_a_rtk_parse);
        g_run("rtk_note", k_g_golden_rtk_note, g_n(k_g_golden_rtk_note), g_a_rtk_note);
    };

    "goldens_recorder_sensitive_pattern"_test = [] {
        g_run("recorder_record", k_g_golden_recorder_record, g_n(k_g_golden_recorder_record),
              g_a_rec_record);
        g_run("recorder_merge", k_g_golden_recorder_merge, g_n(k_g_golden_recorder_merge),
              g_a_rec_merge);
        g_run("sensitive_win", k_g_golden_sensitive_win, g_n(k_g_golden_sensitive_win),
              g_a_sensitive);
        g_run("sensitive_warning", k_g_golden_sensitive_warning,
              g_n(k_g_golden_sensitive_warning), g_a_warning);
        g_run("pattern_newline", k_g_golden_pattern_newline, g_n(k_g_golden_pattern_newline),
              g_a_pattern_newline);
    };
}
