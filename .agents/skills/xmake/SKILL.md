---
name: xmake
description: XMake build configuration, options, commands, and patterns.
---
...
- `xmake build <target1> <target2>` does not work on Windows — each target must be built individually.
- `xmake build -g <glob>` builds targets by group name, not output filename; groups are defined per-target via `set_group()`.
- Sanitizer policies (`build.sanitizer.*`) are preferred over legacy mode rules (`mode.asan`, `mode.tsan`, etc.) because policies propagate to dependent packages and avoid deprecation warnings.
- Sanitizer availability varies by platform and toolchain: verify that a given sanitizer is supported by the target platform before relying on it. For example, TSAN and LSAN are Linux-only; UBSAN's Windows support depends on the clang runtime being properly installed.
- When ASan is used with clang-cl on Windows, the ASan dynamic runtime DLL (`clang_rt.asan_dynamic-x86_64.dll`) must be available in the executable's working directory or PATH at runtime.
- A configuration change (e.g. switching toolchain or enabling a sanitizer) does not automatically trigger a rebuild of already-built targets. Use `-r` or `xmake f -c` to force a clean reconfigure.
- Corrupted or stale executables from a previous build configuration can cause `STATUS_DLL_NOT_FOUND` or `file is corrupted and unreadable` errors — rebuild the affected targets after reconfiguration.

# Xmake Target Writing Tutorial

## Overview

This tutorial covers how to write xmake targets using the standard xmake API, with practical examples. The recommended style uses `on_load` callbacks for dynamic configuration, with static declarations outside.
